...
export const MobileMenu = ({ isOpen, setMenuOpen, categories, isAuthenticated, user, isAdmin, handleLogout }: any) => {  
  ...
  const accessoryCategories = categories.filter((c: any) => !c.slug.includes("ao") && !c.slug.includes("quan"));
  ...
  return (
    <div className="lg:hidden absolute top-full left-0 w-full bg-white border-t shadow-2xl z-50 animate-in slide-in-from-top duration-300 overflow-y-auto max-h-[80vh]">
      <div className="flex flex-col p-6 space-y-2 font-medium">
          ...
          {isAuthenticated ? (
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-2">
                <Link to={ROUTES.PROFILE} onClick={handleLinkClick} className="flex items-center gap-2 p-3 text-sm bg-gray-50 rounded-lg">
                  <User size={16} /> Hồ sơ
                </Link>
                {isAdmin && (
                  <Link to={ROUTES.ADMIN_DASHBOARD} onClick={handleLinkClick} className="flex items-center gap-2 p-3 text-sm bg-purple-50 text-purple-700 rounded-lg">
        ...
        {/* Link đơn lẻ */}
        <Link to="/" onClick={handleLinkClick} className="py-3 border-b border-gray-50 text-brand-dark">
          Sản phẩm mới
        </Link>
        ...
        <div className="border-b border-gray-50">
          ...
          >
            <span>Áo nam</span>
            <ChevronDown size={18} className={`transition-transform ${openSection === 'shirts' ? 'rotate-180' : ''}`} />
          </button>
          
          {openSection === 'shirts' && (
            <div className="pl-4 pb-3 space-y-3 bg-gray-50 rounded-lg mt-1 p-3">
              {shirtCategories.map((sc: any) => (
                <Link key={sc.id} to={`${ROUTES.PRODUCTS}?categorySlug=${sc.slug}`} onClick={handleLinkClick} className="block text-sm text-gray-600">
                  {sc.name}
                </Link>
              ))}
            </div>
        ...
        <div className="border-b border-gray-50">
          ...
          >
            <span>Quần nam</span>
            <ChevronDown size={18} className={`transition-transform ${openSection === 'pants' ? 'rotate-180' : ''}`} />
          </button>
          
          {openSection === 'pants' && (
            <div className="pl-4 pb-3 space-y-3 bg-gray-50 rounded-lg mt-1 p-3">
              {pantCategories.map((pc: any) => (
                <Link key={pc.id} to={`${ROUTES.PRODUCTS}?categorySlug=${pc.slug}`} onClick={handleLinkClick} className="block text-sm text-gray-600">
                  {pc.name}
                </Link>
              ))}
            </div>
        ...
        <div className="border-b border-gray-50">
          ...
          >
            <span>Phụ kiện</span>
            <ChevronDown size={18} className={`transition-transform ${openSection === 'acc' ? 'rotate-180' : ''}`} />
          </button>
          
          {openSection === 'acc' && (
            <div className="pl-4 pb-3 space-y-3 bg-gray-50 rounded-lg mt-1 p-3">
              {accessoryCategories.map((ac: any) => (
                <Link key={ac.id} to={`${ROUTES.PRODUCTS}?categorySlug=${ac.slug}`} onClick={handleLinkClick} className="block text-sm text-gray-600">
                  {ac.name}
                </Link>
              ))}
            </div>
